...
##### :arrow_forward: DISCLAIMER : Certaines des requêtes de ce chapitre peuvent ne pas renvoyer les résultats escomptés
En référer au formateur le cas échéant (Conflits possibles de versions).

- Les term level queries sont les plus utilisées pour les données structurées de type nombre ou date.  
Elles peuvent également être utilisées pour des labels de classification.<br/>

- Par contre elles ne sont pas recommandées pour les recherches textuelles qui ne requiérent pas de correspondance exacte,  
elles sont inopérantes sur des champs analysés.
